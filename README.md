...
- 整页翻译新增轻量进度显示、暂停、取消和失败重试，并继续支持滚动增量补翻。
- 仅译文/双语显示升级为全局偏好，可跨页面、重新翻译和浏览器重启保留。
- 优化整页翻译并发、缓存和动态内容识别，提升长页面翻译速度与稳定性。
- 随身译和随手划加入小译动态状态、快捷控制与更自然的动画反馈。
- 完善自助翻译、英文单词学习、单词本与复制操作。
- 优化多处界面交互，并修复若干显示与稳定性问题。
...
- 自助翻译：在扩展弹窗或助理模式中输入文本，支持自动识别源语言或手动选择源语言与目标语言；输入单个英文单词时直接展示音标、词性、释义、词形和例句。
