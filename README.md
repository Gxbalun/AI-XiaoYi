...
- 助理模式菜单新增 Q 弹展开与收缩动画，并修复三点菜单反复抖动、首次悬停不展开等问题。
- 助理模式入口和快捷按钮整体缩小，降低待机透明度；列表展开或功能窗口打开时自动恢复清晰。
- 单词本增加低饱和黄色底色；关闭功能窗口后保留快捷按钮列表，常用操作更连贯。
- 划词翻译按钮改为跟随鼠标结束位置，支持反向划词和浏览器边缘自动换位。
- 优化“收起小译”选项窗与 30 分钟暂离功能，并统一功能提示框尺寸、移除重复提示。
