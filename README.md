...
- 助理模式入口改用透明背景的小译形象，放大并校正视觉位置；默认保持低透明度，鼠标靠近时恢复清晰，不再触发放大位移。
- 助理模式关闭按钮移到形象外侧，重新绘制居中的关闭图标，并减弱底色和阴影。
- 助理模式快捷按钮缩小间距，常用的整页翻译、自助翻译和单词本保持常驻；鼠标靠近三点按钮时，历史记录、Token 用量和 AI 配置会在同一个按钮框内继续展开。
- 功能窗口打开期间快捷按钮列表保持显示；再次点击同一按钮可以关闭窗口，同时保留当前展开列表，离开后再自然收起。
- 单词本悬浮窗口新增透明度调节，并修复滑块无法连续拖动、松手后调节框意外关闭的问题。
- 取消置顶和透明度按钮的即时黑色提示，仅保留浏览器原生的延迟功能提示。
...
- 划词翻译：选中文本后出现小按钮，点击后以轻量弹窗展示翻译结果；若划选的是单个英文单词，会直接进入单词学习详情。
- 自助翻译：在扩展弹窗或助理模式中输入文本，支持自动识别源语言或手动选择源语言与目标语言后直接翻译。
- 单词学习：仅在点击英文词时请求 AI，展示音标、词性、语境释义、词形和例句；单词详情窗口支持拖动、临时置顶与收藏，查过的词会在本机缓存，避免重复消耗 Token。
- 单词本：收藏词条后可在扩展弹窗或助理模式中搜索、排序、查看、删除或一键清空，不会自动跳转打断阅读。
- 助理模式：浏览器右侧悬浮入口，可快速触发整页翻译、自助翻译、历史记录、单词本、Token 用量和 AI 配置；常用功能保持常驻，低频功能通过三点按钮悬停展开；整页翻译支持通过小圆点快速切换仅译文和双语显示。
- 翻译历史：本地保存最近 7 天历史，可按整页翻译、划词翻译、自助翻译筛选，并支持清空。
- Token 统计：本地记录不同翻译类型的 Token 使用量，支持按用量排序和清空统计。
- 连接测试：保存 AI 配置时自动测试接口是否可用。
- 精致小窗：macOS 风格圆角、轻量图标、自助翻译/历史记录/单词本悬浮窗透明度调节和按钮反馈动画。
